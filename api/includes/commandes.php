...
function getAllCommandes($status = null, $user_id = null, $order = 'ASC') {
    ...
    $query = "SELECT c.*, u.nom AS client_nom, u.prenom AS client_prenom, u.tel AS client_tel, CONCAT_WS(', ', NULLIF(u.rue, ''), NULLIF(u.complement, ''), NULLIF(CONCAT_WS(' ', NULLIF(u.code_postal, ''), NULLIF(u.ville, '')), '')) AS client_adresse 
              FROM Commandes c 
...
function getCommandeById($id) {
    global $pdo;
    $stmt = $pdo->prepare("
        SELECT c.*, u.nom AS client_nom, u.prenom AS client_prenom, u.tel AS client_tel, CONCAT_WS(', ', NULLIF(u.rue, ''), NULLIF(u.complement, ''), NULLIF(CONCAT_WS(' ', NULLIF(u.code_postal, ''), NULLIF(u.ville, '')), '')) AS client_adresse 
        FROM Commandes c
        LEFT JOIN Utilisateurs u ON c.id_client = u.id_user
        WHERE c.id_commande = ?
    ");
